...
import { Plus, Trash2, Settings, X, FileText } from 'lucide-react';
...
interface ReceiptFieldMappingSettingsProps {
  onClose: () => void;
}
...
const RECEIPT_FIELDS = [
  ...
  { value: 'created_at', label: 'Receipt Creation Date' },
];
...
export function ReceiptFieldMappingSettings({ onClose }: ReceiptFieldMappingSettingsProps) {
  ...
  async function loadData() {
    ...
    try {
      const [tagsRes, mappingsRes] = await Promise.all([
        supabase.from('receipt_template_tags').select('*').order('tag_name'),
        supabase
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex justify-between items-center p-6 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-slate-800">Receipt Field Mapping Settings</h2>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-6">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-green-800 space-y-1">
                <li>1. Add template tags that exist in your Receipt PDF template</li>
                <li>2. Map each tag to a field from the receipt data</li>
                <li>3. When generating receipts, tags will be replaced with actual values</li>
              </ul>
            </div>
