...
import { Plus, Trash2, Edit, X, FolderOpen, FileText, Image as ImageIcon, ExternalLink } from 'lucide-react';

interface Resource {
  id: string;
  title: string;
  content: string;
  resource_type: 'text' | 'image' | 'link';
  image_url?: string;
  external_url?: string;
  created_by: string;
  created_at: string;
...
export function ShareResourcesPage() {
  ...
  const [formData, setFormData] = useState({
    title: '',
    content: '',
    resource_type: 'text' as 'text' | 'image' | 'link',
    image_url: '',
    external_url: ''
  });
          ...
          <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                    {formData.resource_type === 'link' ? 'Description' : 'Content'}
                    {formData.resource_type === 'text' && ' *'}
                  </label>
                  <textarea
