...
import { Plus, Trash2, Edit, X, FileText, Image as ImageIcon, ExternalLink, File, Download, Upload as UploadIcon, Mail, Send, Clock, Search, Paperclip, Folder, MessageCircle, CheckCircle } from 'lucide-react';
...
export function ComSecShareResourcesSection() {
  ...
  const [folderPath, setFolderPath] = useState<Array<{id: string, name: string}>>([
    { id: '0AMcQWAT66qySUk9PVA', name: 'ComSec Shared Resources' }
  ]);
  ...
  const fetchDriveFiles = async (folderId?: string) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/browse-drive-files?action=list&folderId=${targetFolderId}&driveType=comsec`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="p-6 border-b border-slate-200 flex items-center justify-between">
              <h3 className="text-xl font-bold text-slate-900">
                Select Files from ComSec Drive
              </h3>
              <button
