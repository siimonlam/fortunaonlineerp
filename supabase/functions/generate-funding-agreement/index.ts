...
async function getServiceAccountToken(): Promise<string> {
  ...
  if (!privateKey) {
    throw new Error("Service account private key not configured for agreement");
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('Generating funding agreement for:', companyName, projectTitle);

    const { data: templateSettings } = await supabase
      .from('system_settings')
      .select('value')
      .eq('key', 'funding_agreement_template_doc_id')
      .maybeSingle();
    ...
    if (!templateDocId) {
      return new Response(
        JSON.stringify({ error: 'Agreement template not configured. Please set funding_agreement_template_doc_id in system settings.' }),
        {
          status: 400,
    ...
    const { data: folderSettings } = await supabase
      .from('system_settings')
      .select('value')
      .eq('key', 'funding_agreement_folder_id')
      .maybeSingle();

    const agreementFolderId = folderSettings?.value;

    if (!agreementFolderId) {
      return new Response(
        JSON.stringify({ error: 'Agreement folder not configured. Please set funding_agreement_folder_id in system settings.' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    const copyResponse = await fetch(`https://www.googleapis.com/drive/v3/files/${templateDocId}/copy?supportsAllDrives=true`, {
      ...
      body: JSON.stringify({
        name: `${agreementNumber} - ${companyName}`,
        parents: [agreementFolderId],
      }),
    });
    ...
    const replacements: Record<string, string> = {
      ...
      '{{AGREEMENT_DATE}}': agreementDate ? new Date(agreementDate).toLocaleDateString('en-GB') : '',
      '{{AMOUNT}}': amount ? `HK$${parseFloat(amount).toFixed(2)}` : '',
    ...
    console.log('Updated document with funding agreement data');
    ...
    console.log('Funding agreement document created:', googleDocUrl);
    ...
    console.error('Error generating funding agreement:', error);
    return new Response(
      JSON.stringify({
        error: error.message || 'Failed to generate funding agreement',
      }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
