/*
  # Fix Periodic Automation - Prevent Retroactive Execution
  
  Updates the execute_periodic_automations function to NOT execute on very old/overdue projects:
  - Only execute if the scheduled date is within the last cycle period
  - Don't add labels/tasks to projects that have been overdue for a long time
  - This prevents spamming all old projects with labels when automation runs
  
  Example:
  - Project started July 2025, frequency 5 days
  - Today is Nov 29, 2025 (4+ months overdue)
  - Should NOT execute - project is too old
  
  - Project started Nov 21, 2025, frequency 5 days  
  - First execution due Nov 26, 2025
  - Today is Nov 29, 2025 (3 days overdue)
  - SHOULD execute - still within reasonable timeframe
*/
...
DECLARE
  ...
  result_json json;
  missed_cycles int;
  max_missed_cycles int := 5; -- Only catch up on max 5 missed cycles
BEGIN
