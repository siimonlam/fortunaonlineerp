/*
  # Fix Periodic Automation to Respect substatus_filter

  1. Changes
    - Update `execute_periodic_automations()` function to check `substatus_filter` field
    - When substatus_filter is set, only execute for projects with that specific status_id
    - When substatus_filter is NULL, use existing logic (main_status or parent_status match)

  2. Notes
    - This allows automations to target specific substatuses like "Mi-Po" under "Hi-Po"
    - Maintains backward compatibility for rules without substatus_filter
*/
...
BEGIN
  ...
  LOOP
    ...
    FOR project_record IN
      SELECT p.id, p.title, p.start_date, p.sales_person_id,
             s.name as status_name, s.parent_status_id
      FROM projects p
      JOIN statuses s ON s.id = p.status_id
      WHERE p.project_type_id = rule_record.project_type_id
        AND (
          -- If substatus_filter is set, match exact status_id
          (rule_record.substatus_filter IS NOT NULL AND p.status_id::text = rule_record.substatus_filter)
          OR
          -- Otherwise use main_status logic (exact match or parent match)
          (rule_record.substatus_filter IS NULL AND (
    ...
    LOOP
      ...
      IF date_field = 'start_date' THEN
        project_start_date := project_record.start_date::date;
      ELSE
        project_start_date := project_record.start_date::date;  -- Default to start_date for now
      END IF;
