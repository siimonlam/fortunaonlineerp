/*
  # Fix Periodic Automation to Use deposit_paid_date Correctly

  1. Changes
    - Update `execute_periodic_automations()` function to properly use deposit_paid_date
    - Add deposit_paid_date to project_record SELECT statement
    - Fix IF/ELSE logic to actually use deposit_paid_date when specified
    
  2. Bug Fixed
    - Previously, even when date_field was 'deposit_paid_date', the function still used start_date
    - Lines 66-70 had ELSE clause that defaulted to start_date instead of using deposit_paid_date

  3. Notes
    - Now properly supports both 'start_date' and 'deposit_paid_date' as date_field values
    - Skips projects where the specified date field is NULL
*/
