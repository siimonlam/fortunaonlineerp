/*
  # Fix Periodic Automation Catch-up Logic
  
  Updates the execute_periodic_automations function to properly handle missed executions:
  - If execution is overdue, calculate the NEXT future execution date from the regular schedule
  - Don't just add frequency days from NOW - that breaks the schedule
  
  Example with frequency = 5 days:
  - Project start: Nov 21, 2025
  - First execution due: Nov 26, 2025 (project_start + 5 days)
  - Today is Nov 29, 2025 (missed the Nov 26 execution)
  - Should execute TODAY and schedule next for: Dec 1, 2025 (not Nov 29 + 5 days)
  
  The key insight: Keep the regular schedule based on project_start_date, not current time.
*/
  ...
  LOOP
    ...
    LOOP
      ...
      ELSIF execution_record.next_execution_at <= now_timestamp THEN
        ...
        -- This keeps the regular schedule intact
